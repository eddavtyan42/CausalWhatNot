...
We evaluate five causal discovery algorithms representing different methodological
approaches: constraint--based (PC, FCI), score--based (GES), continuous optimisation
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lllp{5.5cm}}
    ...
    FCI & Constraint & causal--learn & CI test: same as PC; $\alpha = 0.05$\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{PC and FCI:} The significance level $\alpha$ controls the sparsity of the graph. We selected $\alpha=0.05$ as the standard convention in the literature \cite{Spirtes2000}. We performed a sensitivity analysis (Section~\ref{sec:results_hyperparams}) to confirm that this choice yields a reasonable balance between precision and recall.
...
\begin{itemize}
    \item Python 3.11.5
    \item \texttt{causal-learn} 0.1.3.8 (PC, GES, FCI)
    \item \texttt{CausalNex} 0.11.0 (NOTEARS)
    \item \texttt{numpy} 1.26.0, \texttt{pandas} 2.1.1, \texttt{networkx} 3.1
\end{itemize}
...
\subsubsection{Fast Causal Inference (FCI)}\label{sec:methods_fci}
The FCI algorithm extends PC to handle latent confounders. Instead of a DAG or CPDAG, it outputs a Partial Ancestral Graph (PAG) which represents the Markov equivalence class of maximal ancestral graphs (MAGs).

\textbf{Core Mechanism:}
FCI starts similarly to PC but performs additional conditional independence tests to correctly identify "possible d-separation" sets.
\begin{enumerate}
    \item \textbf{Skeleton:} Similar to PC, but with an extended search for separating sets.
    \item \textbf{Orientation:} Orients v-structures and applies additional rules to identify invariant edge types (e.g., bi-directed edges $\leftrightarrow$ indicating confounding).
\end{enumerate}

\textbf{Implementation Details:}
We use \texttt{causal-learn}'s FCI implementation. We use the same independence tests and $\alpha=0.05$ as for PC. For evaluation, we convert the output PAG to a CPDAG by treating bi-directed edges as undirected edges to allow for SHD comparison, acknowledging this is a simplification.

\textbf{Complexity and Sensitivity:}
FCI is computationally more expensive than PC due to the larger search space for conditioning sets. It is robust to latent confounding but requires larger sample sizes for reliable independence testing.
...
\subsection{Performance of Fast Causal Inference (FCI)}\label{sec:results_fci}
We evaluated FCI on the same benchmarks to assess its performance relative to PC. Since our data generation process did not include latent confounders, FCI should theoretically perform similarly to PC.

\begin{table}[htbp]
  \centering
  \caption{FCI vs PC Skeleton $F_1$ scores.}
  \label{tab:fci_results}
  \begin{tabular}{lcc}
    \toprule
    Dataset & PC $F_1$ & FCI $F_1$ \\
    \midrule
    Asia & 0.84 & 0.82 \\
    Sachs & 0.90 & 0.88 \\
    Alarm & 0.76 & 0.74 \\
...
FCI achieved slightly lower $F_1$ scores than PC across most datasets. This is expected: FCI performs more conditional independence tests (to check for possible d-separation), which increases the probability of Type I errors (false rejections of independence) in finite samples. However, its ability to output PAGs makes it a more robust choice when latent confounding is suspected, even if it incurs a small efficiency cost in unconfounded settings.
