...
\emph{semi-synthetically} (using real-world network structures but simulating fresh data from them) from the known ground--truth graph structure to ensure full
...
For both scenarios we vary the sample size to examine how detection depends on data
 ...
 .  Low stability may indicate spurious edges. We test single-edge omissions/additions; real errors may be multiple and correlated.
...
open--source DAG drawing and analysis tools to guide practitioners. In summary, while algorithms provide a strong baseline, analysts must combine them with diagnostic tests to detect misspecifications, which are reliably flagged for missing edges but harder to isolate for spurious ones.
...
should include algorithms like FCI and RFCI \cite{Spirtes2000, Colombo2012} that learn Partial Ancestral Graphs (PAGs),
